...
// إضافة زر التحدث النصي إلى صفحة محرر النصوص
function addTtsButtonToEssay() {
  const controlGroup = document.querySelector('.control-group');
  if (controlGroup) {
    const ttsButton = document.createElement('button');
    ttsButton.className = 'essay-btn tts-essay-btn';
    ttsButton.innerHTML = `
      <span class="material-symbols-rounded">record_voice_over</span>
      قراءة النص
    `;
    ...
    ttsButton.addEventListener('click', function() {
      const editorText = document.querySelector('.editor-textarea').value.trim();
      if (editorText && typeof responsiveVoice !== 'undefined') {
        // استخدام الإعدادات الافتراضية
        responsiveVoice.speak(editorText, 'Arabic Male', {
          rate: 1,
          pitch: 1,
          onstart: function() {
            ttsButton.innerHTML = `
      ...
      } else if (ttsButton.classList.contains('speaking')) {
        // إيقاف القراءة
        responsiveVoice.cancel();
        resetTtsButton();
      } else {
        alert('الرجاء كتابة نص للقراءة');
      }
    });
    ...
    controlGroup.appendChild(ttsButton);
  }
}
...
// Add scrolling for long text in the essay editor
document.addEventListener('DOMContentLoaded', function() {
  const essayEditor = document.getElementById('essay-editor');
  
  if (essayEditor) {
    // Set max-height and enable scrolling for long content
    essayEditor.style.maxHeight = '500px'; // Adjust height as needed
    essayEditor.style.overflowY = 'auto';
    essayEditor.style.scrollbarWidth = 'thin'; // For Firefox
    
    // Add custom scrollbar styling
    const style = document.createElement('style');
    style.textContent = `
      #essay-editor::-webkit-scrollbar {
        width: 8px;
      }
      
      #essay-editor::-webkit-scrollbar-track {
        background: rgba(0, 0, 0, 0.1);
        border-radius: 4px;
      }
      
      #essay-editor::-webkit-scrollbar-thumb {
        background: rgba(108, 99, 255, 0.5);
        border-radius: 4px;
      }
      
      #essay-editor::-webkit-scrollbar-thumb:hover {
...
// Remove the text-to-speech button
document.addEventListener('DOMContentLoaded', function() {
  // Find and remove the TTS button if it exists
  const ttsButton = document.querySelector('.tts-essay-btn');
  if (ttsButton) {
    ttsButton.remove();
  }
  
  // Also look for any button with the record_voice_over icon
  const voiceButtons = document.querySelectorAll('button span.material-symbols-rounded');
  voiceButtons.forEach(span => {
    if (span.textContent === 'record_voice_over') {
  ...
  // Improve scrollbar styling with purple gradient colors
  const essayEditor = document.getElementById('essay-editor');
  if (essayEditor) {
    // Set max-height and enable scrolling for long content
    essayEditor.style.maxHeight = '500px';
    essayEditor.style.overflowY = 'auto';
    essayEditor.style.scrollbarWidth = 'thin';
    
    // Add custom scrollbar styling with purple gradient
    const style = document.createElement('style');
    style.textContent = `
      #essay-editor::-webkit-scrollbar {
        width: 8px;
      ...
      #essay-editor::-webkit-scrollbar-thumb {
        background: linear-gradient(to bottom, #6047FF, #8c43ff);
        border-radius: 4px;
      }
      
      #essay-editor::-webkit-scrollbar-thumb:hover {
        background: linear-gradient(to bottom, #7057FF, #9c53ff);
      }
    `;
    document.head.appendChild(style);
  }
